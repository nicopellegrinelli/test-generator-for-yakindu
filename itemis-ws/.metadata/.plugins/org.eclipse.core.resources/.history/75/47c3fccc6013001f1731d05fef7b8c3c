...
public class _MotorControlSimplified implements IEventDriven {
    ...
    public _MotorControlSimplified() {
        for (int i = 0; i < 2; i++) {
    ...
    public boolean isStateActive(State state) {
        switch(state) {
            case MOTORCONTROL_MOTORCONTROL:
                return stateVector[0].ordinal() >= State.MOTORCONTROL_MOTORCONTROL.ordinal() && stateVector[0].ordinal() <= State.MOTORCONTROL_MOTORCONTROL_R2_DEFAULT.ordinal();
            case MOTORCONTROL_MOTORCONTROL_R1_BREAKING:
    ...
    private double getSetTargetSpeedValue() {
        if (!setTargetSpeed)
    ...
    private double getSetActualSpeedValue() {
        if (!setActualSpeed)
    ...
    private void raiseSetAcceleration(double value) {
    ...
    private Observable<Double> getSetAcceleration() {
    ...
    private void setTargetSpeed(double value) {
    ...
    private long motorControl_MotorControl_react(long transitioned_before) {
        /* The reactions of state MotorControl. */
    ...
    private long motorControl_MotorControl_r2_Default_react(long transitioned_before) {
        ...
        if (transitioned_after < 1l) {
            if (setTargetSpeed) {
                ...
                enterSequence_MotorControl_MotorControl_r2_Default_default();
                motorControl_MotorControl_react(0l);
        ...
        if (transitioned_after == transitioned_before) {
            ...
            transitioned_after = motorControl_MotorControl_react(transitioned_before);
        }
        return transitioned_after;
    }
