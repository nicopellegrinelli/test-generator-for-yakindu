...
public class Orthogonality implements ICycleBased {
	...
	*/
	...
	/* Default exit sequence for state Cleanup */
	private void exitSequence_main_region_Cleanup() {
		/* Default exit sequence for state Cleanup */
		stateVector[0] = State.$NULLSTATE$;
		stateConfVectorPosition = 0;
	}
	...
	private long main_region_Process_react(long transitioned_before) {
	...
	private long main_region_Process_r2_Line_B_1_react(long transitioned_before) {
		...
		if (transitioned_after==transitioned_before) {
			/* then execute local reactions. */
			transitioned_after = main_region_Process_react(transitioned_before);
		}
		return transitioned_after;
	}
